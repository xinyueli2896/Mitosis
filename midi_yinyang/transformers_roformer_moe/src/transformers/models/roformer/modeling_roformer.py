...
class SWiGLU(Module):
    r"""Applies the Swish-Gated Linear Unit function.

    .. math::
        \text{SWiGLU}(x, V, W, b, c) = (\text{Swish}(xW + b) \otimes (xV + c))

    where :math:`\text{Swish}(x) = x \cdot \sigma(\beta x)` is the Swish/SiLU activation function,
    :math:`\otimes` denotes element-wise multiplication, and :math:`\sigma` is the sigmoid function.

    When bias is disabled:
    .. math::
        \text{SWiGLU}(x, V, W) = \text{Swish}(xW) \otimes (xV)

    Args:
        dim (int): Input dimension for the gating mechanism
        bias (bool, optional): If set to ``False``, the layer will not learn an additive bias.
            Default: ``True``
        beta (float, optional): The :math:`\beta` value for the Swish activation. Default: 1.0

    Shape:
        - Input: :math:`(*, H)` where `*` means any number of additional dimensions
          and `H` must equal to ``dim``
        - Output: :math:`(*, H/2)`
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, bias={self.bias}, beta={self.beta}"
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        if config.hidden_act == "gelu":
            self.fc1 = nn.Linear(config.hidden_size, config.moe_intermediate_size)
            self.fc2 = nn.Linear(config.moe_intermediate_size, config.hidden_size)
            self.activation = nn.GELU()
        else:
            self.fc1 = nn.Linear(config.hidden_size, 2*config.moe_intermediate_size)
            self.fc2 = nn.Linear(config.moe_intermediate_size, config.hidden_size)
...
class GroupedMLP(nn.Module):
    def __init__(self,config):
        ...
        self.num_experts = config.num_experts
        if config.hidden_act =='gelu':
            fc1_output_size = self.config.moe_intermediate_size * self.num_experts
        else:
            fc1_output_size = 2*self.config.moe_intermediate_size * self.num_experts
        fc2_input_size = self.config.moe_intermediate_size * self.num_experts
        self.weight1 = torch.empty(
                        self.config.hidden_size,
                        fc1_output_size,
                        device=torch.cuda.current_device(),
                        dtype=torch.float32,
                        )
        self.weight2 = torch.empty(
                        fc2_input_size,
                        self.config.hidden_size,
                        device=torch.cuda.current_device(),
                        dtype=torch.float32,
    ...
    def forward(self, permuted_local_hidden_states, tokens_per_expert):
        w1 = self.weight1.view(self.num_experts, self.config.hidden_size, -1)
        w2 = self.weight2.view(self.num_experts, -1, self.config.hidden_size)
        fc1_output = gg.ops.gmm(
                permuted_local_hidden_states, w1, tokens_per_expert, trans_b=False
            )
        a, b = fc1_output.chunk(2, dim=-1)
        intermediate_parallel = a * F.silu(b)
        fc2_output = gg.ops.gmm(intermediate_parallel, w2, tokens_per_expert, trans_b=False)
        return fc2_output
class RoFormerTopKRouter(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class RoFormerDispatcher(nn.Module):
    def __init__(self,expert_indices,config):
        super().__init__()
        self.config = config
        self.num_experts = config.num_experts
    ...
    def token_permutation(self,hidden_states,probs,indices):
        ...
        if self.topk>1:
            ...
            # print(f'global_local_mask:{global_local_mask.shape}')
            local_indices = indices.masked_select(global_local_mask)
            self.probs = probs.masked_select(global_local_mask)
            # print(f'global_local_mask:{global_local_mask.shape}')
            absolute_indices = global_local_mask.nonzero()[:, 0]
            self.global_local_map = absolute_indices.view(-1,1).expand(-1, hidden_states.shape[-1])
        ...
        with torch.no_grad():
            ...
            tokens_per_expert = torch.histc(
                ...
                min=self.expert_indices[0],
                max=self.expert_indices[-1],
            )
        ...
        #     # indices[start:end, 0] 是展开后的 token 索引，对应原始 token
        #     orig_token_ids = absolute_indices[self.indices[start:end, 0]]  # 还原原始 token id
        #     print(f"\nexpert {expert_id}: received {count.item()} token copies")
        #     print("original token ids in this block:", orig_token_ids.tolist())
        #     start = end
        return permuted_local_hidden_states,tokens_per_expert
    
    def token_unpermutation(self,hidden_states):
        scores = self.probs.to(dtype=hidden_states.dtype)
        if self.num_experts>1:
        ...
        # print(f'self.hidden_states:{hidden_states}')
        if self.topk>1:
        ...
        # print(f'unpermuterd_local_hidden:{unpermuted_local_hidden.shape}')
        output_total = output_total.view(self.hidden_shape)
        # print(f'output_total:{output_total.shape}')

        # print(f'output:{output_total}')
        return output_total

class RoFormerMoE(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.topk = config.topk
        self.num_experts = config.num_experts
        self.RoFormerTopKRouter = RoFormerTopKRouter(config)
        self.expert_indices = [
    ...
    def forward(self, input:torch.Tensor):
        ...
        expert_output = self.experts(dispathced_input,tokens_per_expert)
...
class ReFormerExpert(nn.Module):
    """single expert network"""
    def __init__(self, config):
        super().__init__()
        self.ffn_first = nn.Linear(config.hidden_size, config.intermediate_size)
        self.ffn_act = nn.GELU()    
        self.ffn_second = nn.Linear(config.intermediate_size, config.hidden_size)
    
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.ffn_first(hidden_states)
        hidden_states = self.ffn_act(hidden_states)
        hidden_states = self.ffn_second(hidden_states)
        return hidden_states

class ReFormerGating(nn.Module):
    """gating network"""
    def __init__(self, config):
        super().__init__()
...
class ReFormerMoe(nn.Module):
    """MoE layer"""
    def __init__(self, config):
        super().__init__()
        self.experts = nn.ModuleList([ReFormerExpert(config) for _ in range(config.num_experts)])
        self.gating = ReFormerGating(config)
        self.num_experts = config.num_experts
        self.topk = config.topk
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # gate = [bs,seq_len,num_experts]
        gate = self.gating(hidden_states)
        # topk_values = [bs,seq_len,topk]  topk_indices = [bs,seq_len,topk]
        topk_values,topk_indices = torch.topk(gate, self.topk, dim=-1)
        topk_values = topk_values / topk_values.sum(dim=-1, keepdim=True)
        # flatten_hidden_states = [bs*seq_len,hidden_size]
        batch_size,seq_len,hidden_size = hidden_states.size()
        flatten_hidden_states = hidden_states.view(-1,hidden_size)
        # flatten_topk_indices = [bs*seq_len*topk]
        flatten_topk_indices = topk_indices.view(-1)
        # 最大容量不能超过topk个token
        expert_capacity = min(int((self.expert_capacity_factor*batch_size*seq_len*self.topk)//self.num_experts)+1,batch_size*seq_len*self.topk)
        # 初始化每个expert的最大容量
        expert_input = torch.zeros(self.num_experts,expert_capacity,hidden_size,device=hidden_states.device)
        expert_ptr = torch.zeros(self.num_experts,dtype=torch.long,device=hidden_states.device)
        # 记录原始位置到专家位置的映射
        ...
        for global_index, expert_id in enumerate(flatten_topk_indices):
            # 对应了token所属的位置
            token_pos = global_index//self.topk
            # 超出了负载 就丢弃该token
            if expert_ptr[expert_id]>=expert_capacity:
                continue
            # 找到对应token的hidden_state
            expert_input[expert_id, expert_ptr[expert_id]] = flatten_hidden_states[token_pos]
            position_map[expert_id, expert_ptr[expert_id]] = token_pos
            expert_ptr[expert_id] += 1
        
        expert_outputs = torch.zeros_like(expert_input)
        for expert_id in range(self.num_experts):
            if expert_ptr[expert_id] > 0:  # 只计算有Token分配的专家
                expert_outputs[expert_id, :expert_ptr[expert_id]] = \
                    self.experts[expert_id](expert_input[expert_id, :expert_ptr[expert_id]])
        final_output = torch.zeros_like(flatten_hidden_states)
        weight_matrix = torch.zeros(
            batch_size * seq_len, 
            self.num_experts, 
            device=hidden_states.device
        )
        weight_matrix.scatter_(
            dim=1, 
            index=topk_indices.view(batch_size*seq_len, self.topk), 
            src=topk_values.view(batch_size*seq_len, self.topk)
        )

        final_output = torch.zeros_like(flatten_hidden_states)
        for expert_id in range(self.num_experts):
            if expert_ptr[expert_id] > 0:
                # 获取该专家处理的所有Token位置
                valid_positions = position_map[expert_id][:expert_ptr[expert_id]]
                valid_positions = valid_positions[valid_positions != -1]
                
                # 计算权重贡献
                weights = weight_matrix[valid_positions, expert_id].unsqueeze(1)  # [num_tokens,1]
                
                # 聚合输出
                final_output[valid_positions] += weights * expert_outputs[expert_id, :len(valid_positions)]
        hidden_states = final_output.view(batch_size,seq_len,hidden_size)
        return hidden_states
    
class RoFormerMoEOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class RoFormerLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.moe:
            ...
            # self.moe_custome = ReFormerMoe(config)
            # group gemm
            # self.moe_custome = RoFormerGroupMoE(config)
            # sequential gemm
            self.moe_custome = RoFormerMoE(config)
            self.moe_output = RoFormerMoEOutput(config)
        else:
    ...
    def forward(
        ...
        if self.moe:
            layer_output = self.moe_custome(attention_output)
            layer_output = self.moe_output(layer_output,attention_output)
        else:
