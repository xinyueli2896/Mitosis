...
class RoFormerSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: torch.Size, past_key_values_length: int = 0) -> torch.Tensor:
        ...
        data = [Tags.PE_POSITIONS, positions, None]  # [str, position_ids, extra_embeddings]
        yield data
        positions = data[1]
        if data[2] is not None:
            return super().forward(positions) + data[2]
        else:
...
class RoFormerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        injected_array = [Tags.HIDDEN_STATES, hidden_states, encoder_hidden_states]
        yield injected_array
        hidden_states, encoder_hidden_states = injected_array[1], injected_array[2]
...
class RoFormerSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        injected_array = [Tags.PRENORM_OUTPUT, hidden_states]
        yield injected_array
        hidden_states = injected_array[1]
...
class RoFormerAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = yield from self.self(
            hidden_states,
            attention_mask,
            sinusoidal_pos,
            head_mask,
        ...
        attention_output = yield from self.output(self_outputs[0], hidden_states)
...
class RoFormerLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = yield from self.attention(
            hidden_states,
            attention_mask,
            sinusoidal_pos,
            head_mask,
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            cross_attention_outputs = yield from self.crossattention(
                attention_output,
                attention_mask,
                sinusoidal_pos,
                head_mask,
...
class RoFormerEncoder(nn.Module):
    ...
    def forward(
        ...
        sinusoidal_pos = (yield from self.embed_positions(hidden_states.shape[:-1], past_key_values_length))[None, None, :, :]
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = yield from layer_module(
                    hidden_states,
                    attention_mask,
                    sinusoidal_pos,
                    layer_head_mask,
