...
\midrule
E1 & A.2 (drum/non-drum) & LA drum $+$ non-drum pair (\code{la\_drum\_cp16\_v2} / \code{la\_nondrum\_cp16\_v2}) & trained (98k) \\
E1 & A.2 (melody/chord) & POP909 melody $+$ chord pair (\code{pop909\_melody\_cp4\_v2} / \code{pop909\_chord\_cp4\_v2}) & in training \\
E1 & B.1 & LA drum $+$ non-drum pair & trained \\
E1 & S0 & LA merged single stream (pretraining) & trained \\
E1 & S1 & POP909 merged, program-tagged (\code{pop909\_melchord\_cp16\_v2}); finetune of S0 & trained (short recipe) \\
\midrule
E2 & A.2 & --- (reuses the E1 checkpoints) & --- \\
E2 & S-mel & POP909 melody-only (\code{pop909\_melody\_cp4\_v2}); finetune of S0, short recipe & \textbf{to train} \\
E2 & S-chord & POP909 chord-only (\code{pop909\_chord\_cp4\_v2}); finetune of S0, short recipe & \textbf{to train} \\
E2 & S-drum / S-nondrum & LA drum-only / non-drum-only (\code{la\_drum\_cp16\_v2} / \code{la\_nondrum\_cp16\_v2}); finetunes of S0 & gated on melchord E2 \\
\midrule
...
E3 & (optional domain match) & Nottingham melody $+$ chord pair (\code{nottingham\_melody\_cp4\_v2} / \code{nottingham\_chord\_cp4\_v2}) for a Nottingham-trained duet, or POP909 pair for a POP909-matched YinYang & optional \\
\midrule
...
E4c & A.2-dense & POP909 melody $+$ chord pair (identical recipe to A.2-melchord) & \textbf{to train} \\
\midrule
