...
def draw_arch(ax, lora=True):
    ...
    ax.text(0.35, 6.1, 'content tokens', fontsize=FS, color=INK, weight='bold', va='center')
    ax.text(0.1, 5.75, r'$\mathrm{LM}_x$  melody', fontsize=FS, color=BLUE, va='center')
    ax.text(0.1, 4.55, r'$\mathrm{LM}_y$  chord', fontsize=FS, color=RED, va='center')
    n = 5
    x0, dx = 1.2, 0.5
    for i in range(n):
    ...
    ax.text(hx + 0.31, 6.1, 'harmonizers', fontsize=FS, color=INK, weight='bold', ha='center', va='center')
    ...
    # attention row
    ay = gy + 1.55
    box(ax, gx + 0.2, ay, 3.2, 0.7, '', fc=GREY_FILL, ec=GREY)
    ax.text(gx + 1.8, ay + 0.48, 'attention of $\\mathrm{LM}_x$ and $\\mathrm{LM}_y$',
            fontsize=FS, ha='center', va='center', color=INK)
    ax.text(gx + 1.8, ay + 0.22, 'pretrained self-attention, per stream',
            fontsize=FS - 1, ha='center', va='center', color=INK_2)
    # gold tabs
    ...
    ax.text(gx + 3.25, fy - 0.05, 'shared expert pool, initialised from the pretrained feed-forward network',
...
def main():
    ...
    draw_masks(axb)
    axa.text(0.0, 6.35, 'a', fontsize=9, weight='bold', color=INK, va='top')
    axb.annotate('b', xy=(-0.34, 1.0), xycoords='axes fraction', xytext=(0, 14),
