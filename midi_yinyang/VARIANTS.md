...
| **+E** (any family) | low-rank experts: ONE shared feed-forward network (the pretrained FFN, fine-tuned unless `MOE_FREEZE_BASE_FFN=1`) plus a rank-r delta per expert on each of its two maps, `expert_e = (W2 + B2_e A2_e) gelu((W1 + B1_e A1_e) x)`, instead of E full copies; routing unchanged; B = 0 at init so step 0 equals the full-copy pool | `MOE_EXPERT_LORA_RANK=r` (`MOE_FREEZE_BASE_FFN=1` optional) | suffix `E<r>` after any `L<r>`, e.g. `A3L16E16` | added 2026-09-23; untrained |
